...
// Доля высоты viewport, на которой стоят ноги монаха. Задана главной сценой:
// в main.png GROUND_Y = 920 из 1116 → 0.8244.
// Все сцены, в которых ходит монах, должны визуально приводить свою
// «плоскость пола» к этой доле — чтобы при переходе (edge-nav, walk-to-edge)
// монах не прыгал вверх/вниз. В сценах с BG, где естественная земля ниже
// этой доли (shelf.png, где трава под шкафом на 0.929), применяется
// вертикальный сдвиг BG+canvas вверх — см. achievements.js.
export const HERO_GROUND_RATIO = 920 / 1116;   // ≈ 0.8244

// Y-координата «пола» для BG с любой высотой — чтобы монах визуально оказался
// на той же доле viewport что и на главной.
export function groundYForBG(bgH) {
  return Math.round(HERO_GROUND_RATIO * bgH);
}
