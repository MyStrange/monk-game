...
function drawHero(){
  if(hero.praying){
    ...
    // Ping-pong: 0→1→2→3→4→3→2→1→0→... для эффекта дыхания
    const cycle = hero.sitFrame % (MONK_FRAMES * 2 - 2); // 0..7
    const f = cycle < MONK_FRAMES ? cycle : (MONK_FRAMES * 2 - 2) - cycle;
...
function playPrayerSound() {
  ...
  // Singing bowl — fundamental + harmonics, long decay
  [[220,0.20,4.0],[440,0.10,3.0],[660,0.055,2.2],[880,0.03,1.6]].forEach(([freq,gain,decay])=>{
...
function loop(){
  ...
  } else {if(tick%50===0)hero.sitFrame++;}
...
function mobilePrayToggle(e) {
  ...
  if (hero.praying) { standUp(); } else { hero.praying = true; playPrayerSound(); }
...
document.addEventListener('keydown',e=>{
  ...
  if(activeScreen!=='main')return;
  if(SIT_K.has(e.key)){hero.praying=true;playPrayerSound();return;}
  ...
  for(const f of bFlies){
    ...
    if(Math.sqrt((f.x-cx)**2+(f.y-cy)**2)<f.sz*5+14){
      ...
      if(jar.caught>=10){
        ...
        // Temporarily zero out caught so jar doesn't look glowy during anim
        jar.caught=0;
        renderHotbar();
...
function startWishAnim(jarRect, onDone){
  ...
  // Use actual canvas dimensions, not potentially-stale bW/bH
  const cW=wishCanvas.width||wishCanvas.offsetWidth||window.innerWidth;
  const cH=wishCanvas.height||wishCanvas.offsetHeight||window.innerHeight;
  wishCanvas.width=cW; wishCanvas.height=cH;

  const bRect=wishCanvas.getBoundingClientRect();
  let ox=cW*0.5, oy=cH*0.88;
  if(jarRect){
    ox=jarRect.left+jarRect.width/2-bRect.left;
    oy=jarRect.top+jarRect.height/2-bRect.top;
  }

  // Hide jar from hotbar during animation so it doesn't show glowing state early
  const jar=getItem('jar')||getItem('jar_open');
  const jarSlotHidden = jar ? inventory.indexOf(jar) : -1;

  const flies=Array.from({length:10},(_,i)=>{
    const travelH = oy + 40;
    const destX = cW*0.1 + Math.random()*cW*0.8;
    ...
    return{
      x:ox+(Math.random()-0.5)*10, y:oy, destX,
  ...
  (function aw(){
    ...
    flies.forEach(f=>{
      ...
      f.x=targetX+windX;
      f.y=oy-progress*progress*(oy+60);
      ...
      if(Math.random()<0.1 && t<0.9){
        dust.push({x:f.x+(Math.random()-0.5)*sz*1.5,y:f.y+(Math.random()-0.5)*sz*1.5,
          vx:(Math.random()-0.5)*0.5,vy:-(0.05+Math.random()*0.2),
          sz:0.8+Math.random()*1.8,age:0,life:35+Math.random()*25,cr,cg,cb});
      }
    });
    ...
    if(af<maxAge){
      ...
      setTimeout(()=>{
        ...
        // Re-sync bW/bH
        const rect=document.getElementById('buddha-screen').getBoundingClientRect();
        bW=bCanvas.width=wishCanvas.width=Math.round(rect.width)||cW;
        bH=bCanvas.height=wishCanvas.height=Math.round(rect.height)||cH;
        // Fresh flies with fade-in
        bFlies=Array.from({length:30},()=>{
          const sz=4+Math.random()*5;
