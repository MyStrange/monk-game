...
// fy/fh — высота секции. Нижний край секции (fy + fh) = SHELF_FLOOR_Y[i]:
// хит-тест клика по полке покрывает пространство ОТ верхней границы секции
// ДО самой планки, где стоят иконки.
const SHELVES = [
  { fx: 0.395, fy: 0.280, fw: 0.210, fh: 0.195 }, // верхняя
  { fx: 0.395, fy: 0.515, fw: 0.210, fh: 0.195 }, // средняя
  { fx: 0.395, fy: 0.755, fw: 0.210, fh: 0.180 }, // нижняя
];

// Y-координата «пола» каждой полки (в BG-норм). Иконки центрируются на
// floorY - size/2, то есть «стоят» нижним краем на этой линии.
// Значения подобраны по видимым деревянным планкам shelf.png — точно
// по верхним краям горизонтальных планок шкафа, чтобы иконки стояли
// на полке как реальные предметы, а не висели в воздухе.
const SHELF_FLOOR_Y = [0.475, 0.710, 0.935];
